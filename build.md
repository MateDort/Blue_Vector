use this for the API current data https://www.ncdc.noaa.gov/cdo-web/webservices/v2#gettingStarted
PROMPT FOR CLAUDE CODE

You are building a Python-based ocean-current-aware routing algorithm for an autonomous, low-energy vessel called Blue Vector. The system should be accurate, modular, and fully usable for simulation and testing.

Objective:
Develop an algorithm that navigates a vessel from a start coordinate to a target coordinate by exploiting real ocean currents and wind, minimizing propulsion energy while meeting arrival constraints.

Requirements:

Data Ingestion

Pull ocean surface current data (u = east velocity, v = north velocity) from NOAA RTOFS (OPeNDAP) or Copernicus Marine Service.

Support bounding boxes and forecast durations gpt predicted ideal season 2 month not ideal 6months but make it calculate accurate delivery time in the algorithm

Store locally as NetCDF for offline use.

Provide a CurrentProvider.get_currents(lat, lon, time) method returning (u, v, forecast_expired) with linear interpolation in space and time.

Optional: Provide mock_data.py with presets (favorable, unfavorable, cross, calm) for testing without internet.

Wind Integration

Use a simple parametric wind model:

Trade winds near equator (westward), westerlies in mid-latitudes (eastward, seasonally varying), smooth transition zones.

Allow configurable sail_efficiency (fraction of wind converted to movement).

Provide wind_model.get_wind(lat, lon, time) returning (u_wind, v_wind).

Vessel Model

Motion equation: position(t+1) = position(t) + current_vector + steering_vector + sail_vector.

Steering vector constrained by MAX_STEERING_SPEED_MS.

Sail vector = sail_efficiency * wind_vector.

Compute propulsion energy per step: E_step = ||steering_vector||^2 * DT_SECONDS.

Convert meters → lat/lon properly (1° lat ≈ 111,320 m; 1° lon ≈ 111,320 × cos(lat)).
...
Sample candidate steering angles within ±ANGLE_RANGE_DEG of bearing.

For each candidate:

Simulate forward HORIZON_STEPS (e.g., 5 days × 4 steps/day).

Sum propulsion energy.

Penalize bad currents: dot product of current vector with target bearing (favorability -1 to 1).

Include α × time, β × remaining distance, optional latitude penalties.

Choose steering angle with lowest total cost.

Apply first step, then repeat at next timestep.

Simulation

Provide simulate_route(start_lat, start_lon, target_lat, target_lon, max_days):

Calls router each timestep.

Moves vessel using vessel model.

Stops when within DISTANCE_TOLERANCE_KM or max_days reached.

Returns:

trajectory (lat/lon list)
...
percent energy saved vs straight-line
...
Use locally stored NetCDF if no internet.

If forecast window expires:

Continue using last known field.

Reduce steering aggressiveness.

Optionally estimate local current from GPS drift minus steering vector.

Visualization

Plot trajectory vs straight-line.

Optional quiver plot of current vectors.

Highlight start (green) and target (red).

Architecture

Modular: data_loader.py, vessel_model.py, router.py, simulator.py, wind_model.py, viz.py, mock_data.py, main.py, server.py.

Clear separation of responsibilities.

Documentation & Explainability

Include inline comments explaining formulas and logic.

Provide “Explain like you’re 8” sections for each module.

Include examples of:

Single step calculation

Choosing steering angle
...
Python 3, clear and readable.

Use xarray for NetCDF handling.

Use standard libraries for math and plotting (numpy, math, matplotlib).

Extra Features (Optional for Hackathon)

Demonstrate good vs bad currents (seasonal differences).

Include wind-only and motor-only scenarios.

Show energy/time savings comparisons.

Mock environment for web demo if no real-time data.

Deliverables:

Working Python code that runs full simulation with currents + wind.

Configurable parameters for testing different scenarios.

Visualization of route, energy usage, and comparison with straight-line baseline.
...
1. **Fetch:** Tries to open **NOAA NOMADS RTOFS** (operational ocean currents) via OPeNDAP: `https://nomads.ncep.noaa.gov/dods/rtofs/rtofs_global{YYYYMMDD}/rtofs_glo_2ds_nowcast_hrly_prog` (2D surface hourly: `u_velocity`, `v_velocity` in m/s). Subsets by bounding box and saves as NetCDF. If today’s run is not yet available, it tries yesterday’s.
...
### 5.1 Decision-making engine (optimization loop)

The following diagram and pseudocode show the **per-step optimization loop** so the decision-making engine is visible at a glance. Implementation: `router.choose_steering`, `router._simulate_forward`, and `simulator.simulate_route`.

**Flowchart (per step until arrival):**

```mermaid
flowchart LR
    subgraph input [Input]
        pos[Current position and time]
        target[Target]
        get_cur[get_currents]
    end
    subgraph opt [Optimization loop]
        bearing[Compute bearing to target]
        angles[Build candidate angles bearing ± range]
        sim[For each angle: _simulate_forward to get cost]
        pick[Choose angle with minimum cost]
    end
    subgraph step [Apply]
        vessel[Vessel step with chosen angle]
        repeat[Repeat until arrival]
    end
    input --> bearing
    bearing --> angles
    angles --> sim
    ...
    pick --> vessel
    vessel --> repeat
    repeat -->|next step| input
```

**Pseudocode:**

```
function choose_steering(lat, lon, target_lat, target_lon, current_time, get_currents):
    bearing := bearing_deg(lat, lon, target_lat, target_lon)
    steering_ref := 90 - bearing
    candidates := [steering_ref - angle_range .. steering_ref + angle_range] in steps of angle_step

    best_angle := steering_ref
    best_cost := infinity
    for each angle in candidates:
        cost, _, _ := _simulate_forward(lat, lon, current_time, target, angle, get_currents, HORIZON_STEPS, alpha, beta, ...)
        if cost < best_cost:
            best_cost := cost
            best_angle := angle
    return (best_angle, best_cost)

function simulate_route(start, target, max_days, current_provider, ...):
    trajectory := [start]
    time := start_time
    while distance to target > tolerance and elapsed < max_days:
        angle, _ := choose_steering(lat, lon, target_lat, target_lon, time, get_currents, ...)
        (lat, lon, ...) := step(lat, lon, currents, angle, ...)
        trajectory.append((lat, lon))
        time += timestep
    return trajectory, times, total_days, total_energy, percent_saved, mode, sail_fraction
```
